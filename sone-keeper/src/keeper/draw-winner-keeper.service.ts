...
export class DrawWinnerKeeperService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    const chainId = Number(
      this.configService.get<string>('CHAIN_ID') || '50312',
    ); // Monad testnet

    if (!privateKeyRaw || !soneAddress) {
      this.logger.error(
        'Missing environment variables for keeper. Check KEEPER_PRIVATE_KEY and SONE_ADDRESS.',
      );
      return;
    }
    ...
    this.logger.log(
      `[KEEPER_KURO] Keeper service initialized with account: ${this.account.address}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `[KEEPER_KURO] Keeper wallet balance: ${formatEther(balance)} MON`,
      );
      ...
      if (balance < BigInt(10000000000000000)) {
        // 0.01 MON
        this.logger.warn(
          `[KEEPER_KURO] Low keeper wallet balance! Current balance: ${formatEther(balance)} MON`,
        );
        this.logger.warn(
          `[KEEPER_KURO] Please fund the account ${this.account.address} with some MON for gas fees.`,
        );
      }
    } catch (error) {
      this.logger.error(
        `[KEEPER_KURO] Error checking keeper wallet balance: ${error.message}`,
      );
    }
  }
  ...
  async checkAndDrawWinner() {
    if (this.transactionInProgress) {
      this.logger.debug(
        '[KEEPER_KURO] Transaction in progress, skipping check...',
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        '[KEEPER_KURO] Checking if a winner needs to be drawn...',
      );

      const kuryoAddress = this.configService.get<string>('SONE_ADDRESS');
      this.logger.debug(
        `[KEEPER_KURO] Using contract address: ${kuryoAddress}`,
      );
      ...
      try {
        ...
        this.logger.log(
          `[KEEPER_KURO] Current block: ${blockNumber}, timestamp: ${new Date(Number(block.timestamp) * 1000).toISOString()}`,
        );
      } catch (error) {
        this.logger.warn(
          `[KEEPER_KURO] Could not get block info: ${error.message}`,
        );
      }
      ...
      const [status, startTime, endTime, , numberOfParticipants] =
        roundInfo as unknown as readonly [
          number,
          number,
          number,
          number,
      ...
      this.logger.log(
        `[KEEPER_KURO] Round Status: ${status} (${RoundStatus[status]})`,
      );
      this.logger.log(
        `[KEEPER_KURO] Start Time: ${new Date(Number(startTime) * 1000).toISOString()}`,
      );
      this.logger.log(
        `[KEEPER_KURO] End Time: ${endTimeSet ? new Date(Number(endTime) * 1000).toISOString() : 'Not set'}`,
      );
      this.logger.log(
        `[KEEPER_KURO] Current Time: ${new Date(now * 1000).toISOString()}`,
      );
      this.logger.log(
        `[KEEPER_KURO] Number of Participants: ${numberOfParticipants}`,
      );
      ...
      this.logger.log(
        `[KEEPER_KURO] - Has Minimum Players: ${hasMinimumPlayers}`,
      );

      if (isOpen && endTimeSet && isPastEndTime && hasMinimumPlayers) {
        this.logger.log(
          '[KEEPER_KURO] ✅ All conditions met! Round is ready for winner drawing!',
        );
        ...
        this.logger.log(
          `[KEEPER_KURO] Current balance: ${formatEther(balance)} MON`,
        );
        this.logger.log(
          `[KEEPER_KURO] Minimum required balance: ${formatEther(minimumBalance)} MON`,
        );

        if (balance < minimumBalance) {
          this.logger.error(
            `[KEEPER_KURO] ❌ Insufficient balance to pay for gas!`,
          );
          this.logger.error(
            `[KEEPER_KURO] Current balance: ${formatEther(balance)} MON`,
          );
        ...
        this.logger.log(
          `[KEEPER_KURO] ✅ Account balance sufficient: ${formatEther(balance)} MON. Enough for ~${Number(balance / minimumBalance)} more rounds.`,
        );

        try {
          this.logger.log(
            '[KEEPER_KURO] 🚀 Attempting to call drawWinner function...',
          );
          ...
          try {
            ...
            this.logger.warn(
              `[KEEPER_KURO] Could not estimate gas: ${gasError.message}`,
            );
          }
          ...
          try {
            ...
            this.logger.warn(
              `[KEEPER_KURO] Could not get nonce: ${nonceError.message}`,
            );
          }
          ...
          this.logger.log(
            `[KEEPER_KURO] ✅ Winner drawn! Transaction hash: ${hash}`,
          );

          this.logger.log(
            '[KEEPER_KURO] Waiting for transaction confirmation...',
          );
          const receipt = await this.client.waitForTransactionReceipt({ hash });
          this.logger.log(
            `[KEEPER_KURO] ✅ Transaction confirmed! Block: ${receipt.blockNumber}, Gas used: ${receipt.gasUsed}`,
          );
          ...
          if (
            ...
            if (error.message.includes('Winner already drawn for this round')) {
              this.logger.warn(
                '[KEEPER_KURO] ⚠️ Winner was already drawn for this round',
              );
              ...
              this.logger.error(
                '[KEEPER_KURO] ❌ Nonce error - transaction may be stuck',
              );
            } else {
              this.logger.error(
                `[KEEPER_KURO] ❌ Unknown contract error: ${error.message}`,
              );
            }
          } else {
        ...
        this.logger.warn(
          '[KEEPER_KURO] ⚠️ End time passed but not enough participants',
        );
        this.logger.warn(
          `[KEEPER_KURO] Number of participants: ${numberOfParticipants} (minimum required: 2)`,
        );
        ...
        try {
          ...
          this.logger.log(
            `[KEEPER_KURO] ✅ Round cancelled! Transaction hash: ${hash}`,
          );

          const receipt = await this.client.waitForTransactionReceipt({ hash });
          this.logger.log(
            `[KEEPER_KURO] ✅ Transaction confirmed! Block: ${receipt.blockNumber}`,
          );
        } catch (error) {
        ...
        this.logger.debug(
          '[KEEPER_KURO] Conditions not met for drawing winner:',
        );
        if (!isOpen) {
          this.logger.debug(
            `[KEEPER_KURO] - Round is not open (status: ${status})`,
          );
        }
        if (!endTimeSet) {
        ...
        if (!isPastEndTime) {
          this.logger.debug(
            `[KEEPER_KURO] - Round has not ended yet (current: ${now}, end: ${Number(endTime)})`,
          );
        }
        if (!hasMinimumPlayers) {
          this.logger.debug(
            `[KEEPER_KURO] - Not enough participants (${numberOfParticipants} < 2)`,
          );
        }
      }
    } catch (error) {
